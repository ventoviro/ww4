...
class EdgeRenderer extends AbstractEngineRenderer
{
     ...
     *
     * @return EdgeRenderer Return self to support chaining.
     */
    ...
    public function render(string $layout, array $data = [], array $options = []): string
    {
        ...
        return $this->getEngine([], true)->render($layout, (array) $data);
    }
    ...
    public function getCache()
    {
        if (!$this->cache) {
            if ($this->config->exists('cache_path')) {
                $this->cache = new EdgeFileCache($this->getOption('cache_path'));
            } else {
